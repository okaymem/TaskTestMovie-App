...
<MovieCard genres='Comedy, Drama, Romance' 
 ...
 <Pagination color="#9854F6" total={500} value={page} onChange={pageChanger} style={{margin: '24px 112px 82px auto'}}/>
 </div>
...
// function Head() {
//  const [page, pageChanger]=useState(1)
//  let [totalPages, totalPagesChanger]=useState(1)
//const [ratedMoviesList, setRatedMovie]=useState( JSON.parse(localStorage.getItem('ratedMoviesList')) || [])
...
//  async function getContent() {
//    try {
//        const response = await fetch(`http://localhost:3001/?genre=${gen}&page=${page}&rateFrom=${rateFrom}&rateTo=${rateTo}&sortby=${sortby}&year=${year}`);
//        const { results } = await response.json();
//        console.log(results);
//        chcont(results)
//    } catch (error) {
...
//  async function getGenres() {
//    try {
//        const response = await fetch(`http://localhost:3001/getgenres`);
//        const genres = await response.json();
//        console.log(genres);
//        setGenres(genres.genres.map(genre => ({value:genre.id, label:genre.name})))
//    } catch (error) {
...
//    <h1 style={{marginRight:'auto',marginLeft:'113px'}}>Movies</h1>
//    <div className={classes.selects}>
 //  <div><p className={classes.sortTitle}>Genres</p><Select title='Select genre' selectChanger={handleCurrencyChange} selectType={genres}/></div>
 //  <div><p className={classes.sortTitle}>Release year</p><Select title='Select release year' selectChanger={yearchanger} selectType={years}/></div> 
 //  <div><p className={classes.sortTitle}>Ratings</p><Select style={{width: '3px'}}  title='From' selectType={ratings} selectChanger={ratefromChanger}/></div> 
 //  <div  className={classes.sortTo}><Select title='To' selectType={ratings} selectChanger={ratetoChanger}/></div> 
 //   <button onClick={() => {
 ...
 //  <div className={classes.sortSelect}><p className={classes.sortTitle}>Sort by</p><Select  selectChanger={sortbyChanger} selectType={sortbyvariants} title='Sort by'/></div> 
//    </div>
//    <div className={classes.wrapper}>{content.map(element=>
//
//<MovieCard genres='Comedy, Drama, Romance' 
//  rating={element.vote_average? element.vote_average.toString().slice(0,3) : '0'}
//  id={element.id}
...
// 
// 
// <Pagination total={500} value={page} onChange={pageChanger} />
// </div>
//    
//  );
//}
...
//  .then(response=> setGenres(response.genres.map(genre => ({value:genre.id, label:genre.name}))))
//  .catch(error => console.error(error));
//fetch(`https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=en-US&page=${page}${rateFrom}${rateTo}${gen}${year}${sortby}`, options)
//  .then(response => response.json())
//  .then(response => {
//    chcont(response.results)
//    //
// // totalPagesChanger(response.total_pages)
//  })
...
//
// const ratefromChanger = (value) => {
//  rateFromHandle(`&vote_average.gte=${value}`)
// }
// const ratetoChanger = (value) => {
//  rateToHandle(`&vote_average.lte=${value}`)
// }
//  
// const sortbyChanger = (value) => {
//  if (value==='Most Popular'){
//    sortbyHandler('&sort_by=popularity.desc')
//  } else if (value==='Least Popular'){
//    sortbyHandler('&sort_by=popularity.asc')
//  } else if (value==='Most Rated'){
//  sortbyHandler('&sort_by=vote_average.desc')
//} else if (value==='Least Rated'){
//  sortbyHandler('&sort_by=vote_average.asc')
//  } else if (value==='Most Voted'){
//  sortbyHandler('&sort_by=vote_count.desc')
//  } else {
//    sortbyHandler('&sort_by=vote_count.asc') 
//  }
// }
//
//  return (
